...
Elem* QueueMCSP<Elem>::dequeue() {
    ...
    long long num = __sync_fetch_and_add(&maxNum, 1);// tmp = *ptr; *ptr = value; return tmp;
    ...
    if (!head->isTail) {
		firstElement = __sync_val_compare_and_swap(&head, head, head->next); //if the current value of *ptr is oldval, then write newval into *ptr.
